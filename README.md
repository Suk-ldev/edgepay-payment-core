...
第三方收单插件的商户号、门店号、终端号和码牌 ID 不要求管理员凭字段名猜测。先保存
平台登录凭据，让目标码牌真实完成一笔小额收款，再在插件编辑区点击“查询最近流水”。
系统按插件运行能力由 Worker 直接查询，或把两分钟有效的任务交给在线 Docker Watcher；
查询时会清空已有编号筛选，只返回近 5 分钟成功流水的时间、金额、支付方式和配置编号，
不返回付款人或备注。管理员选中目标流水后可一键填入当前插件声明的编号字段，核对并保存。
完整操作和平台字段对照见 [`../../docs/plugins/00-receipt-identifiers.md`](../../docs/plugins/00-receipt-identifiers.md)。
